...
static void* sendto_thread(void* thread_data) {
  ...
  while (!client->terminated) {
    ...
    if (res == -1) {
      fprintf(stderr, "Can't send addr %s\n", client->name_addr);
    }
  }
...
struct client_t* client_init(const char* inter_name,
  ...
  if (!client) {
    fprintf(stderr, "client_init malloc %s\n", server_addr);
    goto aborting;
  }
  ...
  if (!hosten) {
    perror("client_init gethostbyname");
    return NULL;
  }

  struct in_addr** addr_list = (struct in_addr**)hosten->h_addr_list;
  if (addr_list[0] == NULL) {
    fprintf(stderr, "client_init incorrect addres %s\n", server_addr);
    return NULL;
  }
...
int client_run(struct client_t* client) {
  ...
  if (res != 0) {
    fprintf(stderr, "ERROR> client_run pthread_create addres: %s port: %d \n",
            client->name_addr, client->port);
  }
